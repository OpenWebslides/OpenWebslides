...
RSpec.describe 'Password API', :type => :request do
  ...
  describe 'POST /' do
    ...
    it 'accepts unconfirmed users' do
      post password_path, :params => request_body(unconfirmed_user.email), :headers => headers

      expect(response.status).to eq 204
    end

    it 'accepts invalid emails' do
      post password_path, :params => request_body('foo'), :headers => headers

      expect(response.status).to eq 204
    end

    it 'requests a password reset token' do
      post password_path, :params => request_body(user.email), :headers => headers
  ...
  describe 'PUT/PATCH /' do
    ...
    it 'resets a password' do
      ...
      patch password_path, :params => reset_body(token, new_password), :headers => headers
