...
RSpec.describe 'Notification API', :type => :request do
  ...
  describe 'GET /' do
    ...
    it 'returns a list of all notifications' do
      ...
      expect(notification['attributes']).to include 'predicate'
      expect(notification['attributes']).to include 'subjectDisplayName'
      expect(notification['attributes']).to include 'deckDisplayName'
  ...
  describe 'GET /:id' do
    ...
    it 'returns a notification' do
      ...
      expect(notification['attributes']).to include 'predicate'
      expect(notification['attributes']).to include 'subjectDisplayName'
      expect(notification['attributes']).to include 'itemDisplayName'
      expect(notification['meta']).to include 'createdAt'
    end
  end
end
