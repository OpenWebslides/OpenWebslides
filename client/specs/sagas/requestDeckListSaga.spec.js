...
describe('Request for deck list Saga', () => {
  describe('Request list of own decks flow', () => {
    ...
    it('has a happy path', () => {
      ...
      expect(generator.next().value).toEqual(call(requestUserDecksCall, 12));

      const resultListOfDecks = [];

      for (let i = 0; i < 12; i += 1) {
        resultListOfDecks.push({
          id: dummyResponse[i].data[0].id,
          name: dummyResponse[i].data[0].attributes.name,
          description: dummyResponse[i].data[0].attributes.description,
        });
      }
